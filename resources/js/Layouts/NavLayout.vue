...
<template>
    <div class="relative">
        <div id="TopNav" class="w-[100%] h-[60px] fixed bg-black z-20 flex items-center justify-between">
            ...
            <div>
                <img class="rounded-full mx-8" width="35"
                    src="https://yt3.ggpht.com/yti/ANjgQV-DpAsAUiajS1V9kTT_74uF2R34gNbB0l7JmcTGhVaZw5A=s88-c-k-c0x00ffffff-no-rj">
            </div>
        </div>

        <div v-if="width > 639">
            <div>
                <div id="SideNav" v-if="$page.url === '/'" :class="[!openSideNav ? 'w-[70px]' : 'w-[240px]']"
                    class="h-[100%] fixed z-0 bg-black w-[240px]">
                    <ul :class="[!openSideNav ? 'p-2' : 'px-5 pb-2 pt-[7px]']" class="mt-[60px] w-full">

                        <Link :href="route('home')">
                        <SideNavItem :open-side-nav="openSideNav" iconString="Home" />
                        </Link>
                        <Link :href="route('addVideo')">
                        <SideNavItem :open-side-nav="openSideNav" iconString="Add Video" />
                        </Link>
                        <Link :href="route('deleteVideo')">
                        <SideNavItem :open-side-nav="openSideNav" iconString="Delete Video" />
                        </Link>


                        <div class="border-b border-b-gray-700 my-2.5"></div>
                        <SideNavItem :open-side-nav="openSideNav" iconString="Subscriptions" />
                        <SideNavItem :open-side-nav="openSideNav" iconString="Library" />
                        <SideNavItem :open-side-nav="openSideNav" iconString="Liked" />
                        <SideNavItem :open-side-nav="openSideNav" iconString="History" />
                        <SideNavItem :open-side-nav="openSideNav" iconString="Watch Later" />
        ...
        <div @click="openSideNavOverlay = false" class="bg-black bg-opacity-70 fixed z-50 w-full h-screen" :class="openSideNavOverlay
            ...
            : 'hidden z-[-1]'
            " />
        <div id="SideNavOverlay" ref="sideNavOverlay" class="h-[100%] fixed z-50 bg-black w-[240px] mt-[9px]"
            :class="openSideNavOverlay ? 'animate__animated animate__slideInLeft animate_faster' : 'animate__animated animate__slideOutLeft animate__faster'">
            <!--iki tane _ -->
            <div class="flex items-center">
            ...
            <ul class="w-full px-5 py-2 p-2 mt-2 ">
                <<Link :href="route('home')">
                    <SideNavItem :open-side-nav="true" iconString="Home" />
                    </Link>
                    <Link :href="route('addVideo')">
                    <SideNavItem :open-side-nav="true" iconString="Add Video" />
                    </Link>
                    <Link :href="route('deleteVideo')">
                    <SideNavItem :open-side-nav="true" iconString="Delete Video" />
                    </Link>
                    <div class="border-b border-b-gray-700 my-2.5"></div>
                    <SideNavItem :open-side-nav="true" iconString="Subscriptions" />
                    <SideNavItem :open-side-nav="true" iconString="Library" />
                    <SideNavItem :open-side-nav="true" iconString="Liked" />
                    <SideNavItem :open-side-nav="true" iconString="History" />
                    <SideNavItem :open-side-nav="true" iconString="Watch Later" />
                    <div v-if="openSideNav">
                        <div class="border-b border-b-gray-700 my-2.5" />
                        <div class=" text-gray-400 text-[14px] text-extrabold">
                            About Press Copyright
